...
<div class="mobileMenu <?php $main_menu_dropdown_overview = get_theme_mod( 'uw-madison-wp-2015_dropdowns_overview_id' ); if($main_menu_dropdown_overview == "enabled") { echo "showOverview"; } else { echo "hideOverview"; } ?>">
...
<?php if(!get_theme_mod( 'uw-madison-wp-2015_alert_enabled_id' )) { ?>


<!-- The Important Message banner that goes across the top of the site to display important information and alerts -->
<div class="important-message-container <?php if(get_theme_mod( 'uw-madison-wp-2015_alert_sticky_id' )) { ?>sticky<?php } ?>">

<?php if(!get_theme_mod( 'uw-madison-wp-2015_alert_dismiss_id' )) { ?>
	<button class="dismiss-alert"><svg height="28px" style="enable-background:new 0 0 28 28;" version="1.1" viewBox="0 0 28 28" width="28px" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><g id="Layer_1"/><g id="x"><g><polygon points="28,22.398 19.594,14 28,5.602 22.398,0 14,8.402 5.598,0 0,5.602 8.398,14 0,22.398     5.598,28 14,19.598 22.398,28   " /></g></g></svg><span>Dismiss Alert</span></button>
<?php } ?>
    <div>
        <h1>Important</h1>
    </div>
    <div>
		<p>For the latest updates on UW–Madison plans and responses related to the COVID-19 pandemic, visit covid19.wisc.edu.</p> <p>Please note visitors are not allowed in UW facilities and employees are working remotely.</p>
		<p><?php echo get_theme_mod( 'uw-madison-wp-2015_alert_message2_id' ); ?> </p>
    </div>
    <div>
        <a href="https://covid19.wisc.edu">
            <button class="important-message-button">Visit covid19.wisc.edu</button>
        </a>
    </div>
</div>

<script>
	let alertdismiss = sessionStorage.getItem('alertdismiss');

	if(alertdismiss === "hide") {
		document.querySelector(".important-message-container").style.display="none";
	}

	document.querySelector(".dismiss-alert").addEventListener("click", function() {
		document.querySelector(".important-message-container").style.display="none";
		sessionStorage.setItem('alertdismiss', 'hide');
	});
</script>

<style>
/* Styles for Important Message banner */
...
.important-message-container.sticky {
	position: sticky;
	top: 0px;
	z-index:20;
}
...
@media only screen and (max-width: 680px) {
  ...
  div.important-message-container .dismiss-alert {
	top: 1rem;
	transform: unset;
  }
}

.important-message-container .dismiss-alert {
	position: absolute;
	top: 50%;
...
.important-message-container .dismiss-alert svg {
	width: 15px;
	height: 15px;
}

.important-message-container .dismiss-alert span {
	opacity: 0;
	font-size: 0px;
}

.important-message-container .dismiss-alert svg polygon {
	fill: rgba(255,255,255,0.4);
}

.important-message-container h1 {
  margin: .1em .5em;
  font-size: 1.8em;
  letter-spacing: 1px;
...
.important-message-container a {
  text-decoration: underline;
  color: #6bb5ff;
}

.important-message-container a:hover {
...
.important-message-container div {
  margin: 0 1.8em;
}
...
.important-message-button {
  ...
  border: 0;
  color: #fff;
  background: linear-gradient(45deg, #2972E0, #4F95FF);
  text-shadow: none;
  font-size: .8em;
  letter-spacing: 1px;
}
.important-message-button:hover {
  background: linear-gradient(45deg, #5c5c5c, #808080);
  box-shadow: none;
}
...
<div id="page" style="<?php echo $page_width_value; ?>" class="hfeed site <?php echo $page_width_class; ?> <?php $editor = get_field('page_editor'); if($editor == "advanced") { echo "apesetup"; } ?> <?php $header_slides_style = get_theme_mod( 'uw-madison-wp-2015_header_slides_options_id' ); if($header_slides_style == "hidden") { echo "hiddenSlides"; } else { echo "visibleSlides"; } ?> <?php $header_style = get_theme_mod( 'uw-madison-wp-2015_header_style_options_id' ); if($header_style == "opaque") { echo "solidHeader"; } else if($header_style == "photo") { echo "photoHeader"; } else { echo "photoHeader"; } ?> <?php $home_layout_settings = get_theme_mod( 'uw-madison-wp-2015_layout_id' ); if($home_layout_settings == "tiles") { echo "tiledPosts"; }; if($home_layout_settings == "right_sidebar") { echo "postsRightSidebar"; } ?> <?php $header_layout_settings = get_theme_mod( 'uw-madison-wp-2015_header_layout_id' ); if($header_layout_settings == "righthand") { echo "rightHandNav"; } else { echo "navBar"; } ?> <?php if( get_field('hide_the_side_navigation')) { echo "hiddenSidebar"; } ?> <?php if( get_field('page_theme') == 'documentation') { echo 'documentationSection'; } ?> <?php $sidebar_settings = get_theme_mod( 'uw-madison-wp-2015_sidebar_style_id' ); if($sidebar_settings == "card") { echo 'cardSidebar'; } else if($sidebar_settings == "dots") { echo 'dotsSidebar'; } else { echo 'lineSidebar'; } ?><?php $fontselection = get_theme_mod( 'uw-madison-wp-2015_fonts_id' ); echo ' '.$fontselection; ?> <?php if(get_field('featured_pages_layouts')) { $page_layout = get_field('featured_pages_layouts'); if($page_layout == 'listr') { echo 'listRightSidebar'; }  } ?> <?php $header_slides_dots = get_theme_mod( 'uw-madison-wp-2015_headerslides_dots_id' ); if($header_slides_dots == true) { echo "hiddenSlideDots"; } ?> <?php $header_slides_arrows = get_theme_mod( 'uw-madison-wp-2015_headerslides_arrows_id' ); if($header_slides_arrows == true) { echo "hiddenSlideArrows"; } ?> <?php $header_slides_transition = get_theme_mod( 'uw-madison-wp-2015_headerslides_transition_id' ); if($header_slides_transition == true) { echo "headerSlidesFade"; } ?> <?php $logo_position = get_theme_mod( 'uw-madison-wp-2015_home_logo_position_id' ); if($logo_position == "center") { echo "hidemainlogo"; } ?> <?php $navtextsize = get_theme_mod( 'uw-madison-wp-2015_nav_text_size_id' ); if($navtextsize == "large") { echo "largeNavText"; } ?> <?php $hideshownav = get_theme_mod( 'uw-madison-wp-2015_hide_show_nav_id' ); if($hideshownav == "yes") { echo "hideMainNav"; } $post_title_mod = get_theme_mod('uw-madison-wp-2015_post_featured_setting_id'); if($post_title_mod == "title_above") { echo " postTitleAbove"; } ?>">
    ...
    <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd" sketch:type="MSPage">
        <g id="Desktop-HD-Alt" sketch:type="MSArtboardGroup" transform="translate(-1242.000000, -34.000000)" fill="#FFFFFF">
			...
			<nav id="site-navigation" class="main-navigation <?php $main_menu_dropdown = get_theme_mod( 'uw-madison-wp-2015_dropdowns_id' ); if($main_menu_dropdown == "enabled") { echo "dropdownEnabled"; } ?> <?php $main_menu_dropdown_overview = get_theme_mod( 'uw-madison-wp-2015_dropdowns_overview_id' ); if($main_menu_dropdown_overview == "enabled") { echo "showOverview"; } else { echo "hideOverview"; } ?>" role="navigation">
